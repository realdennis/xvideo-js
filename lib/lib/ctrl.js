...
function homeXv() {
	...
	obj.page = 0; //initial
	obj.videoList = [];
	obj.nextPage = _asyncToGenerator(function* () {
		try {
			let temp = yield xvideo.hpc(obj.page);
	...
	obj.right = _asyncToGenerator(function* () {
		try {
			if (obj.videoList[obj.pointer].tag == undefined) {
				obj.videoList[obj.pointer].tag = 'Waiting...';
				obj.renderTen();
				let tag = yield xvideo.tagCrawl(obj.videoList[obj.pointer].attr.link);
	...
	obj.save = _asyncToGenerator(function* () {
		if (obj.videoList[obj.pointer].tag == undefined) {
			obj.videoList[obj.pointer].tag = 'Waiting...';
			obj.renderTen();
			let tag = yield xvideo.tagCrawl(obj.videoList[obj.pointer].attr.link);
...
exports.keypage = function (key) {
	...
	obj.nextPage = _asyncToGenerator(function* () {
		try {
			let temp = yield xvideo.kwc(obj.keyword, obj.page);
			obj.videoList = obj.videoList.concat(temp);
			obj.page += 1;
		} catch (err) {
...
exports.favpage = function () {
	...
	obj.delete = function () {
		...
		try {
			for (let i = obj.index; i < obj.index + 10; i++) {
				...
				if (i == obj.pointer) console.log('->	Delete');else console.log('  	' + obj.videoList[i].attr.name);
			}
	...
	obj.renderTen = function () {
		...
		try {
			for (let i = obj.index; i < obj.index + 10; i++) {
				if (obj.videoList[i] == undefined) break;
				if (i == obj.pointer) console.log('->	' + chalk.red.bold(obj.videoList[i].attr.name));else console.log('  	' + obj.videoList[i].attr.name);
			}
			console.log('==========================================================');
			console.log('"right" : See the video tag');
			console.log('"enter" : Watch the video');
